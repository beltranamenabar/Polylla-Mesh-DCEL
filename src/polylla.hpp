...
class Polylla {
    ...
    void construct_Polylla() {
        ...
        for(auto &e : seed_edges) {
            ...
            if(!has_BarrierEdgeTip(poly)) { //If the polygon is a simple polygon then is part of the mesh
                ...
                barrieredge_tip_reparation(poly);
            }
        }
        t_end = std::chrono::high_resolution_clock::now();
        elapsed_time_ms = std::chrono::duration<double, std::milli>(t_end - t_start).count();
    ...
    void print_ALE(std::string filename) {
        ...
        ///Find borderedges
        int b_curr, b_init = 0;
        for(int i = tr->halfEdges() - 1; i != 0; i--) {
    ...
    bool has_BarrierEdgeTip(std::vector<int> poly) {
        int length_poly = static_cast<int>(poly.size());
        int x, y, i;
    ...
    void barrieredge_tip_reparation(std::vector<int> &poly) {
        int x, y, i;
        int t1, t2;
